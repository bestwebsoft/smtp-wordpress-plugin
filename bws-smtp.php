	...
	function bwssmtp_settings_page() {
		...
		if ( isset( $_REQUEST['bws_restore_confirm'] ) && check_admin_referer( $plugin_basename, 'bws_settings_nonce_name' ) ) {
			...
			$bwssmtp_notices['bwssmtp_restore_result'] = array(
				'type'  => 'success',
				'text'  => __( 'All plugin settings were restored.', 'bwssmtp' )
			);
		} ?>
		<div class="wrap">
			...
			<?php if ( ! $bwssmtp_options['settings_changed'] ) { ?>
				<div class="updated fade"><p><strong><?php _e( 'Configure plugin for sending email messages via SMTP. For more info see', 'bwssmtp' ); ?>&nbsp;<a target="_blank" href="https://docs.google.com/document/d/1zCvr7FarorqcggQC1PcyjHaxadrCgS3-CMMw3JsvW1M/edit#heading=h.jxgxn2x6c109"><?php _e( 'plugin instruction', 'bwssmtp' ); ?></a>.</strong></p></div>
			<?php }
			...
			if ( ! isset( $_GET['action'] ) ) { 
				if ( isset( $_REQUEST['bws_restore_default'] ) && check_admin_referer( $plugin_basename, 'bws_settings_nonce_name' ) ) {
					...
					<form id="bwssmtp_settings_form" method="post" action="admin.php?page=bwssmtp_settings" autocomplete="on">
						<table class="form-table">
							<tbody>
								...
								<tr class="bwssmtp_settings_area" valign="top">
									...
									<td>
										<div>
											<input id="bwssmtp_area_where_selected" type='radio' name="bwssmtp_area" value="where_selected" <?php if ( $bwssmtp_options['area'] == "where_selected" ) echo 'checked="checked"'; ?> maxlength="250" /><label for="bwssmtp_area_where_selected"><?php _e( 'Where Selected', 'bwssmtp' ); ?></label>
										</div>
										<div>
...
if ( ! function_exists ( 'bwssmtp_plugin_banner' ) ) {
	function bwssmtp_plugin_banner() {
				...
				<div class="updated bwssmtp_message" style="padding: 0; margin: 0; border: none; background: none;">
					<div class="bws_banner_on_plugin_page">
						...
						<div class="text" style="margin-top: 32px;">
							<?php _e( 'Configure the "SMTP by BestWebSoft" plugin for sending email messages via SMTP', 'bwssmtp' ); ?>
						</div>
						<div class="button_div">
							<a class="button" href="admin.php?page=bwssmtp_settings"><?php _e( 'Go to the settings', 'bwssmtp' ); ?></a>
						</div>
					</div>
				</div>
		<?php }
...
add_action( 'admin_notices', 'bwssmtp_plugin_banner' );
/* Uninstall plugin. */
